...
class SVGNodes:
	...
	def _gen_node(node, fill):
		x, y = NODES[node]
		return (f'<circle cx="{x}" cy="{y}" r="15" style="fill:{fill}"/>\n'
	...
	def get_svg_group(self):
		frame = ('<g style="fill:#000000;stroke:#000000;stroke-width:1;font:bold 17px sans-serif;text-anchor:middle">\n', '\n</g>\n')
		data = '\n'.join(self._nodes)
...
if __name__ == '__main__':
	...
	ap.add_argument('-s', '--svg', help='svg node file (default: nodes.svg)', type=str, default=INFILE)
	ap.add_argument('-o', '--outfile', help='svg output (default: svg file with random tail)', type=str, default=None)
