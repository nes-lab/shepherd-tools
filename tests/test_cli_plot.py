...
def test_cli_plot_file_full(data_h5_path):
    res = CliRunner().invoke(
        cli, ["-vvv", "plot", "--start", "0", "--end", "8", "--width", "50", "--height", "10", str(data_h5_path)]
    )
...
def test_cli_plot_file_short(data_h5_path):
    res = CliRunner().invoke(
        cli, ["-vvv", "plot", "-s", "2.345", "-e", "8.765", "-w", "30", "-h", "20", str(data_h5_path)]
    )
    assert res.exit_code == 0
...
def test_cli_plot_dir_min(tmp_path):
    ...
    res = CliRunner().invoke(
        cli, ["-vvv", "plot", str(tmp_path.absolute())]
    )
    assert res.exit_code == 0
...
def test_cli_multiplot_dir_full(tmp_path):
    ...
    res = CliRunner().invoke(
        cli, ["-vvv", "plot", "--start", "1", "--end", "7", "--width", "40", "--height", "10", "--multiplot", str(tmp_path)]
    )
    assert res.exit_code == 0
...
def test_cli_multiplot_dir_short(tmp_path):
    ...
    generate_h5_file(tmp_path, "hrv_file2.h5")
    res = CliRunner().invoke(
        cli, ["-vvv", "plot", "-s", "2.345", "-e", "8.765", "-w", "30", "-h", "20",  "-m", str(tmp_path)]
    )
    assert res.exit_code == 0
