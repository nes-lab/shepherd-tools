...
@pytest.mark.parametrize("model_type", tb_client.query_types())
def test_fixtures_visibility(model_type: str) -> None:
    ...
    if model_type.lower() not in content_supported:
        return
    content_names = tb_client.query_names(model_type)
    for content_name in content_names:
        model_data = tb_client.query_item(model_type, name=content_name)
...
def test_fixtures_are_components_or_content() -> None:
    ...
    fixture_types = tb_client.query_types()
    assert len(set(fixture_types)) == len(fixture_types)
    assert len(set(fixture_types) & c_and_c) == len(set(fixture_types))


@pytest.mark.parametrize("model_type", set(tb_client.query_types()) & set(content_supported.keys()))
...
@pytest.mark.parametrize(
    "model_type", set(tb_client.query_types()) & set(components_supported.keys())
)
