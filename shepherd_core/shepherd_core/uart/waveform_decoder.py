...
def detect_inversion(events: np.ndarray) -> bool:
    ...
    """
    events = events[:1000, :]  # speedup for large datasets
...
def prepare_events(events: np.ndarray) -> np.ndarray:
    ...
    if len(events) > len(events_clean):
        print(
            f"filtered out {len(events) - len(events_clean)}/{len(events)} events (redundant)"
        )
    ubr = detect_baudrate(events_clean)
    print("detected baud-rate = %d", ubr)
    if detect_half_stop(events_clean):
        print("detected 1.5-stop")
...
def decode_uart(
    ...
    bit_oder: BitOrder = BitOrder.lsb_first,
) -> np.ndarray:
    """
    ways to detect EOF:
    - long pause on HIGH
    ...
    symbol_events = np.concatenate(content).reshape((len(content), 2))
    # symbol_events[:, 0] = symbol_events[:, 0].astype(float)  # does not work
    return symbol_events


def get_line_events(symbol_events: np.ndarray) -> np.ndarray:
    content = []
    semi_stop = False
    t_start = None
    line = ""
    for time, symbol in symbol_events:
        if symbol == "\n" or semi_stop:
            if symbol == "\n":
                line += symbol
    ...
    return np.concatenate(content).reshape((len(content), 2))


def get_text(line_events: np.ndarray) -> str:
    return "".join(line_events[:, 1])
...
if __name__ == "__main__":
    ...
    sym = decode_uart(uev)
    lne = get_line_events(sym)
    txt = get_text(lne)
    # print(txt)

    print(
        "t_open=",
        timeit("open_csv(Path('uart_raw2.csv'))", globals=globals(), number=1000),
    )
    print("t_prep=", timeit("prepare_events(uev)", globals=globals(), number=1000))
    print("t_deco=", timeit("decode_uart(uev)", globals=globals(), number=1000))
    print("t_line=", timeit("get_line_events(sym)", globals=globals(), number=1000))
    print("t_text=", timeit("get_text(lne)", globals=globals(), number=1000))
    """ Results: file with 15.5k events (2700 symbols, 61 lines)
    t_open= 5.309 [ms/run]
    t_prep= 0.354
    t_deco= 72.607  [!!!!!]
    t_line= 4.179
